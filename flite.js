...
export function Flite(renderer3, config) {
    let {THREE, renderer, scene, camera, glbLoader, controls, events, rendering,directionalLight} = renderer3;
    ...
    let makeGround = () => {
        ...
        tex.colorSpace = 'srgb'
        let mesh = new THREE.Mesh(new THREE.BoxGeometry(gx * 2,gy * 2,gz * 2),new THREE.MeshStandardMaterial({
            color: '#ffffff',
            map: tex
        }));
    ...
    this.init = async () => {
        ...
        */
        ...
        let planeUpdate = (p) => {
            if (keyboard.KeyE)
                p.throttle = lerp(p.throttle, 1, .1);
            if (keyboard.KeyQ)
                p.throttle = lerp(p.throttle, 0, .1);
            if (keyboard.KeyA)
                p.rollInput = lerp(p.rollInput, -1, .01);
            if (keyboard.KeyD)
                p.rollInput = lerp(p.rollInput, 1, .01);
            if (keyboard.KeyW)
                p.pitchInput = lerp(p.pitchInput, -1, .01);
            if (keyboard.KeyS)
                p.pitchInput = lerp(p.pitchInput, 1, .01);
            ...
            if (abs(v0.dot(v)) < 100.)
                p.body.applyLocalImpulse(v0.multiplyScalar(1.1), v1.set(0, 0, 1));
        ...
        events.frame = (dt) => {
            ...
            if (time >= frameTime) {
                ...
                let endFrameNumber = frameNumber + elapsedFrames;
